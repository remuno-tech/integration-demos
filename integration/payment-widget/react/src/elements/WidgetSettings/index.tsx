...
const WidgetSettings = () => {
  ...
  useEffect(() => {
    if (widgetParams.merchantId && widgetParams.apiKey) {
      fetch(
        `https://api.remuno.com/v1/merchants/${widgetParams.merchantId}/coins`,
        { headers: { 'x-api-key': widgetParams.apiKey } },
      )
        .then((res) => res.json())
  ...
  const handleSelectCoin = (coin: string) => {
    ...
    if (isInList) {
      dispatch(
        setSelectedCoins(widgetParams.selectedCoins.filter((c) => c !== coin)),
      );
    } else {
    ...
    <VStack w='50%' h='100vh' p={2} overflow='auto' spacing='8px'>
      ...
      <FormControl>
        ...
        <Input
          ...
          value={widgetParams.notificationUrl}
      ...
      <FormControl>
        ...
        {widgetParams.isEnabledSummary && (
          <>
            <Input
              mt={2}
              value={widgetParams.summary.template}
            ...
            {summaryVariables?.map((variable) => (
              <Box mt={2}>
                ...
                <Input
                  ...
                  onChange={(e) =>
                    dispatch(
                      setSummaryVariables({ [variable.name]: e.target.value }),
                    )
                  }
                />
              </Box>
